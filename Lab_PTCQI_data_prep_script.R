...
files <- list.files(path = '/Users/Bhuti/Documents/SiteLevel/lab-tool')
temp <- lapply(files, fread, sep='\t', colClasses=list(character=1:46)) # read all the files into another list
df <- rbindlist(temp) # combine them to one datafram
...
df.lab2 <- df.lab2 %>% filter(!OperatingUnit %in% c('Kenya', 'Ethiopia', 'Nigeria', 'South Africa', 'Tanzania', 'Uganda',
                                                    'Zambia', 'Cameroon', "Cote d'Ivoire", "Zimbabwe", 'Democratic Republic of the Congo'))

write.xlsx(df.lab2, 'Other-OUs.xlsx', asTable = F)
